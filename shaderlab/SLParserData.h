...
namespace shaderlab
{
	...
	struct SLSubProgram {
		...
		struct ParamVector {
			...
			ParamVector(ShaderParamType type, const std::string &name, int index, int dimension)
			...
			{
				return type == rhs.type && name == rhs.name && index == rhs.index && dimension == rhs.dimension;
			}
			...
			std::string toString()
			{
				return FormatString("VectorParam(name=%s,type=%s,index=%d,dimension=%d)", name.c_str(), GetShaderParamType(type).c_str(), index, dimension);
			}
		};
		...
		struct ParamMatrix {
			...
			ParamMatrix(int idx, int rowCount, const std::string &name)
				: index(idx)
				, rowCount(rowCount)
				, name(name)
			{
			...
			{
				return name == rhs.name && index == rhs.index && rowCount == rhs.rowCount;
			}

			std::string toString()
			{
				return FormatString("MatrixParam(name=%s,index=%d,rowcount=%d)", name.c_str(), rowCount, index);
			}
		};
		...
		void AddParamVector(ShaderParamType type, int index, int dimension, const std::string &name);
		void AddParamMatrix(int index, const std::string &name, int rowCount);
		...
		void Bind(const char* src, const char* dest);
		...
		std::string toString()
		{
			...
			if (bindChannelMap.size() > 0) {
				ret += "    BindChannel:\n        ";
				for (auto it = bindChannelMap.begin(); it != bindChannelMap.end(); ++it) {
					ret += FormatString("%-10s > %-10s", it->first.c_str(), it->second.c_str()) + "\n        ";
				}
				ret += "\n";
			}
		...
		std::map<std::string, std::string> bindChannelMap;
	};
