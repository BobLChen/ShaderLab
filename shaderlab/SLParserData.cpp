...
namespace shaderlab
{
	...
	void SLSubProgram::Bind(const char* src, const char* dest)
	{
		bindChannelMap.insert(std::make_pair<std::string, std::string>(src, dest));
	}
	...
	void SLSubProgram::AddParamVector(ShaderParamType type, int index, int dimension, const std::string &name)
	{
		std::vector<ParamVector>& params = constantBuffers.empty() ? vectorParams : constantBuffers.back().vectorParams;
		params.emplace_back(type, name, index, dimension);
	}

	void SLSubProgram::AddParamMatrix(int index, const std::string &name, int rowCount)
	{
		std::vector<ParamMatrix>& params = constantBuffers.empty() ? matrixParams : constantBuffers.back().matrixParams;
		params.emplace_back(index, rowCount, name);
	}
